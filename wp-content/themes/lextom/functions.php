...
require_once LEXTOM_DIR . '/inc/cpt-products.php';




function lextom_create_default_products() {
    $existing = get_posts( array( 'post_type' => 'lextom_product', 'numberposts' => 1 ) );
    if ( ! empty( $existing ) ) return;
    
    // VIP BED
    $id = wp_insert_post( array( 'post_title' => 'VIP BED', 'post_content' => 'Autonómna platforma starostlivosti — automatizovaná hygiena, polohovanie, monitoring a zero-gravity technológia.', 'post_status' => 'publish', 'post_type' => 'lextom_product' ) );
    if ( $id ) { wp_set_object_terms( $id, 'Inteligentná starostlivosť', 'lextom_product_cat' ); update_post_meta( $id, '_lextom_product_tag', 'Nové' ); update_post_meta( $id, '_lextom_product_coming_soon', '0' ); update_post_meta( $id, '_lextom_product_tagline', 'Mobilná hygienická stanica' ); }

    // Soon products
    for ( $i = 2; $i <= 8; $i++ ) {
        $sid = wp_insert_post( array( 'post_title' => 'Produkt #' . $i, 'post_excerpt' => 'Informácie budú doplnené. Pripravujeme ďalšie inovatívne riešenia pre oblasť starostlivosti.', 'post_status' => 'publish', 'post_type' => 'lextom_product' ) );
        if ( $sid ) { wp_set_object_terms( $sid, 'Nový produkt', 'lextom_product_cat' ); update_post_meta( $sid, '_lextom_product_tag', 'Pripravujeme' ); update_post_meta( $sid, '_lextom_product_coming_soon', '1' ); }
    }
}
add_action( 'admin_init', 'lextom_create_default_products' );

function lextom_update_first_product() {
    $existing = get_posts( array( 'post_type' => 'lextom_product', 'numberposts' => 1, 'orderby' => 'ID', 'order' => 'ASC' ) );
    if ( ! empty( $existing ) ) {
        $product = $existing[0];
        // Only update if it still has the old title
        if ( $product->post_title === 'VIP BED' ) {
            $updated_post = array(
                'ID'           => $product->ID,
                'post_title'   => 'Prenosný sprchový a sušiaci systém',
                'post_excerpt' => 'Integrované prenosné zariadenie pre kompletnú hygienu pacienta bez potreby kúpeľne, externého prívodu vody či elektriny. Umožňuje šetrné sprchovanie aj sušenie priamo na lôžku alebo v kresle — ideálne pre imobilných a čiastočne mobilných klientov. Mikro-rozprašovacia technológia zabezpečuje dôkladné, no úsporné čistenie pri minimálnej spotrebe vody.',
                'post_content' => ''
            );
            wp_update_post( $updated_post );
            
            wp_set_object_terms( $product->ID, 'Hygiena a starostlivosť', 'lextom_product_cat' );
            update_post_meta( $product->ID, '_lextom_product_tagline', 'Bezvodá mobilná hygiena — sprchovanie a sušenie kdekoľvek.' );
            update_post_meta( $product->ID, '_lextom_product_spec1_val', '4,95 kg' );
            update_post_meta( $product->ID, '_lextom_product_spec1_lbl', 'Hmotnosť' );
            update_post_meta( $product->ID, '_lextom_product_spec2_val', '1,7 L' );
            update_post_meta( $product->ID, '_lextom_product_spec2_lbl', 'Nádrž na vodu' );
            update_post_meta( $product->ID, '_lextom_product_spec3_val', '7500 mAh' );
            update_post_meta( $product->ID, '_lextom_product_spec3_lbl', 'Batéria' );
        }
    }
}
add_action( 'admin_init', 'lextom_update_first_product' );

function lextom_force_update_first_product() {
    if ( get_option( 'lextom_first_product_updated' ) ) return;
    $existing = get_posts( array( 'post_type' => 'lextom_product', 'numberposts' => 1, 'orderby' => 'ID', 'order' => 'ASC' ) );
    if ( ! empty( $existing ) ) {
        $product = $existing[0];
        $updated_post = array(
            'ID'           => $product->ID,
            'post_title'   => 'Prenosn� sprchov� a su�iaci syst�m',
            'post_excerpt' => '',
            'post_content' => 'Integrovan� prenosn� zariadenie pre kompletn� hygienu pacienta bez potreby k�pelne, extern�ho pr�vodu vody ci elektriny. Umo�nuje �etrn� sprchovanie aj su�enie priamo na l��ku alebo v kresle � ide�lne pre imobiln�ch a ciastocne mobiln�ch klientov. Mikro-rozpra�ovacia technol�gia zabezpecuje d�kladn�, no �sporn� cistenie pri minim�lnej spotrebe vody.'
        );
        wp_update_post( $updated_post );
        
        wp_set_object_terms( $product->ID, 'Hygiena a starostlivost', 'lextom_product_cat' );
        update_post_meta( $product->ID, '_lextom_product_tagline', 'Bezvod� mobiln� hygiena � sprchovanie a su�enie kdekolvek.' );
        update_post_meta( $product->ID, '_lextom_product_spec1_val', '4,95 kg' );
        update_post_meta( $product->ID, '_lextom_product_spec1_lbl', 'Hmotnost' );
        update_post_meta( $product->ID, '_lextom_product_spec2_val', '1,7 L' );
        update_post_meta( $product->ID, '_lextom_product_spec2_lbl', 'N�dr� na vodu' );
        update_post_meta( $product->ID, '_lextom_product_spec3_val', '7500 mAh' );
        update_post_meta( $product->ID, '_lextom_product_spec3_lbl', 'Bat�ria' );
        
        update_option( 'lextom_first_product_updated', 1 );
    }
}
add_action( 'admin_init', 'lextom_force_update_first_product' );
